...
pub fn listen(socket: *sock, url: []u8) void {
  ...
  if(c.nng_listen(socket.*, util.sliceToCstr(std_allocator, url),
                   @intToPtr([*c]c.struct_nng_listener_s, 0), 0) != 0) {
...
pub fn dial(socket: *sock, url: []u8) void {
  ...
  if(c.nng_dial(socket.*, util.sliceToCstr(std_allocator, url),
                 @intToPtr([*c]c.struct_nng_dialer_s, 0), 0) != 0) {
